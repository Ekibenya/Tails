...
内嵌 **310 条**设定条目，分十二卷，每轮按关键词、在场人物、当前地点与纪年分层注入。
...
1. 打开在线地址，或用浏览器打开本仓库的 `index.html`。
...
本仓库就是可以直接部署的静态站，`vercel.json` 已经写好：

```
index.html               整个游戏（内嵌卡数据与世界书）
core/res/icon/           PWA 图标
manifest.webmanifest     PWA 清单
sw.js                    离线缓存
vercel.json              缓存头（core/ 长缓存，index.html 不缓存）
docs/media/              本文档的截图
```

在 Vercel 上 Import 这个仓库，框架选 **Other**，构建命令留空，输出目录填 `.`，即可。

## 关于三维

本版是纯文字的旁观版，不加载三维引擎，也不带资材包——玩家实际要下载的只有一份 `index.html` 加几张图标，打开不向站外发一次请求。（`docs/` 只是这份文档的截图，不参与运行。）
...
- 页面本身除 Vercel 内置的匿名访问统计（`/_vercel/insights`，与部署平台同源）之外不发出任何外部请求；自行部署到别处时这一条会 404，不影响运行。
